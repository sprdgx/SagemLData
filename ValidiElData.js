class ValidiElData {
    ...
    ZidWchThab(name, validationFunction) {
      this.customValidations[name] = validationFunction;
    }
  
    async validiEZ(data, rules) {
      const errors = [];
  
      const validateField = async (fieldName, fieldValue, fieldRules) => {
      ...
      for (const fieldName in rules) {
        ...
        const validationPromise = validateField(fieldName, fieldValue, fieldRules);
    ...
    validi(data, rules) {
      const validationResult = this.validiEZ(data, rules);
      return validationResult;
    }
  }
  
  module.exports = ValidiElData;
